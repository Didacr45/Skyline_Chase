...
func _physics_process(delta: float) -> void:
	# Si está muerto, salimos antes de mover nada
...
func take_damage(amount: int) -> void:
	...
	print("Papito, al enemigo le queda: ", current_health)

	if current_health <= 0:
		_die()

func _on_player_attack(damage: int) -> void:
	# Solo recibe daño si el jugador está detectado y no está muerto
...
func _die() -> void:
	...
	# 1. APAGADO TOTAL: Ni física, ni choques, ni señales
	set_physics_process(false)
	velocity = Vector3.ZERO
	collision_layer = 0
	collision_mask = 0
	
	# 2. ESCONDER UI: Quitamos la barra para que no flote
	var hb = get_node_or_null("HealthBar3D")
	if hb: hb.hide()
	
	# 3. ANIMACIÓN: Stop para que no mezcle con 'Walk' y luego Death
	if anim_player:
		anim_player.stop() 
