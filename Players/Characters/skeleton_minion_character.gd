...
func _physics_process(delta: float) -> void:
	# Si está muerto, ni gravedad ni movimiento, papito
...
# ─────────────────────────────────────────
#  SISTEMA DE VIDA Y MUERTE (BLOQUEADO)
# ─────────────────────────────────────────
...
func take_damage(amount: int) -> void:
	...
	print("Minion dañado, papito. Vida: ", current_health)
...
func _die() -> void:
	...
	# 3. ANIMACIÓN DE MUERTE LIMPIA
	if anim_player:
		anim_player.stop() # Corta la de caminar de golpe
...
func _start_random_movement() -> void:
	# Bucle seguro que se rompe al morir
